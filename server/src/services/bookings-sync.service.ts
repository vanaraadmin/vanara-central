import { beds24Get, beds24GetAbsolute, type Beds24Bindings } from "./beds24-client.service.js";
...
interface Beds24Booking {
  ...
  allowReview?: string;
  cancellation?: { type?: string; daysBeforeArrival?: number };
  bookingTime?: string;
  modifiedTime?: string;
  ...
  bookingGroup?: Array<{ id?: number; bookingId?: number; isMaster?: boolean; [key: string]: unknown }>;
  [key: string]: unknown;
}
...
async function upsertBooking(
  ...
  if (!propertyId || !roomTypeId || !booking.arrival || !booking.departure) return 0;
  ...
  `).bind(
    ...
    text(booking.city), text(booking.state), text(booking.postcode), text(booking.country), text(booking.countryCode),
    ...
    text(booking.allowReview), text(booking.cancellation?.type), numberOrNull(booking.cancellation?.daysBeforeArrival),
    text(booking.bookingTime), text(booking.modifiedTime), text(booking.cancelTime), JSON.stringify(booking), syncedAt, syncedAt,
  ).run();

  const local = await env.DB.prepare("SELECT booking_id FROM bookings WHERE beds24_booking_id = ?")
    .bind(booking.id).first<{ booking_id: number }>();
  if (!local) return result.meta?.changes ?? 0;

  const children: D1PreparedStatement[] = [
    env.DB.prepare("DELETE FROM booking_guests WHERE booking_id = ?").bind(local.booking_id),
    env.DB.prepare("DELETE FROM booking_info_items WHERE booking_id = ?").bind(local.booking_id),
  ];
  for (const guest of booking.guests ?? []) {
    children.push(env.DB.prepare(`
    ...
    `).bind(
      local.booking_id, numberOrNull(guest.id), text(guest.title), text(guest.firstName), text(guest.lastName),
      text(guest.email), text(guest.phone), text(guest.mobile), text(guest.company), text(guest.address), text(guest.city),
      text(guest.state), text(guest.postcode), text(guest.country), text(guest.countryCode), text(guest.flagText),
  ...
  for (const item of booking.infoItems ?? []) {
    const code = text(item.code);
    if (!code) continue;
    children.push(env.DB.prepare(`
      INSERT INTO booking_info_items (booking_id, info_code, info_name, info_value, raw_json, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).bind(local.booking_id, code, text(item.name), item.value == null ? null : String(item.value), JSON.stringify(item), syncedAt, syncedAt));
  }
  const childResults = await env.DB.batch(children);
  return (result.meta?.changes ?? 0) + childResults.reduce((sum, row) => sum + (row.meta?.changes ?? 0), 0);
}
...
export async function syncBookings(env: BookingsSyncBindings): Promise<BookingsSyncResult> {
  ...
  try {
    ...
    while (true) {
      pagesRead += 1;
      if (response.success === false) throw new Error(response.error ?? "Beds24 bookings returned success=false");
      for (const booking of response.data ?? []) {
        recordsRead += 1;
        const changes = await upsertBooking(env, booking, maps, startedAt);
        if (changes === 0) recordsSkipped += 1;
        else recordsWritten += changes;
      }
    ...
    await env.DB.batch([
      ...
      env.DB.prepare(`
        ...
        VALUES ('bookings', ?, ?, 'success', ?, ?, ?, NULL)
      `).bind(startedAt, finishedAt, recordsRead, recordsWritten, recordsSkipped),
    ]);
    return { ok: true, mode, modifiedFrom, pagesRead, recordsRead, recordsWritten, recordsSkipped, startedAt, finishedAt };
  } catch (error) {
    const finishedAt = new Date().toISOString();
    const message = error instanceof Error ? error.message : "Unknown bookings sync error";
    try {
      await env.DB.prepare(`
        INSERT INTO sync_runs (sync_type, started_at, finished_at, status, records_read, records_written, records_failed, error_message)
        VALUES ('bookings', ?, ?, 'failed', ?, ?, 1, ?)
      `).bind(startedAt, finishedAt, recordsRead, recordsWritten, message).run();
    } catch (logError) { console.error("Unable to log bookings sync failure", logError); }
    throw error;
  }
}
